#include <stdlib.h> // atoi, rand, qsort, malloc, abs
#include <stdio.h>
...
/*
gcc efficient_convex_hull.c
./a.out n > 스크립트 명(convex.R같은)
*/
////////////////////////////////////////////////////////////////////////////////
...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
	...
	if (num_point < 1){
		...
		// upper hull을 구한다.
		lines = upper_hull( s11, n11, p1, points[max_index], lines, num_line, capacity); //upper hull
		lines = upper_hull( s12, n12, points[max_index], pn, lines, num_line, capacity); //lower hull
...
// 실제로는 sqrt는 계산하지 않음 분모는 상수니까 비교만 할거면 굳이 계산 안해도 ok.
...
// ****왼쪽을 살리고, 오른쪽은 버려. p1->pn직선에서 가장 먼 점 pm이 있을 때. p1->pm, pm->pn직선 모두 오른쪽은 convex hull이 될 수 없음. 안쪽이니깐. ****
// [output] s1 : 직선의 upper(left)에 속한 점들의 집합 (ax+by-c < 0) 왼쪽 
// [output] s2 : lower(right)에 속한 점들의 집합 (ax+by-c > 0) 오른쪽
// [output] n1 : s1 집합에 속한 점의 수
// [output] n2 : s2 집합에 속한 점의 수

// !! from, to가 s1,s2에 속하지 않게 해야함! 안그러면 오류 혹은 무한루프!!
// from, to랑 좌표가 같은 점이면 (소숫점때문에?) 그건 안들어가게 하기.
// ax+by+c == 0 으로 하면 위험. 0.00006 이러면 위험. 
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2){
	int a, b, c;
	*n1 = 0;
	*n2 = 0;
	...
	for (int i=0; i<num_point; i++){
		if ( !((points[i].x==from.x && points[i].y==from.y)||(points[i].x==to.x && points[i].y==to.y)) ){
			if ( a*points[i].x+b*points[i].y-c < 0 ){ //-c맞아???? left++
				s1[*n1].x = points[i].x;
				s1[*n1].y = points[i].y;
				(*n1)++;
			}
	...
	fprintf(stderr, "-----------------\n%d %d %d!\n", a, b, c);
	for (int i=0; i<(*n1); i++){
		fprintf(stderr, "%d %d || %d \n", s1[i].x, s1[i].y, a*s1[i].x+b*s1[i].y-c );
	}
	fprintf(stderr, "\n");
	for (int i=0; i<(*n2); i++){
		fprintf(stderr, "%d %d || %d\n", s2[i].x, s2[i].y, a*s2[i].x+b*s2[i].y-c);
	}
	fprintf(stderr, "-----------------\n");
	*/
}
...
t_line *convex_hull( t_point *points, int num_point, int *num_line)
{
	...
	*num_line = 0;
	...
	// 점들을 분리 points[0]가 가장 왼쪽, points[num_point-1]이 가장 오른쪽 점.
	fprintf(stderr, "p1: %d %d pn: %d %d\n", points[0].x, points[0].y, points[num_point-1].x, points[num_point-1].y);
...
int main( int argc, char **argv)
{
	...
	// sort the points by their x coordinate. x축 기준으로 정렬.
