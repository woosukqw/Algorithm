...
for var in {1..300}; do ./a.out 500 > conv.R && sleep .5 && ./conv.R && mv convex.png convex$var.png && sleep .5 && mv convex$var.png results; done
*/
////////////////////////////////////////////////////////////////////////////////
...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
	...
	if (num_point == 0){
		...
		if (max_index == -1) return lines;

		t_point *s11 = (t_point*)malloc(sizeof(t_point)*num_point);
		t_point *s12 = (t_point*)malloc(sizeof(t_point)*num_point);
		t_point *npe11 = (t_point*)malloc(sizeof(t_point)*num_point);
		t_point *npe12 = (t_point*)malloc(sizeof(t_point)*num_point);

		int n11, n12, nope1, nope2; // number of points in s1, s2, respectively

		separate_points( points, num_point, p1, points[max_index], s11, npe11, &n11, &nope1);
		separate_points( points, num_point, points[max_index], pn , s12, npe12, &n12, &nope2);
		...
		free(s11); free(s12); free(npe11); free(npe12);
	}
