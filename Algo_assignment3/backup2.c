...
static void dfs_main( int initial_state, int goal_state, int depth, int visited[]){
	...
	if (initial_state == goal_state) {
		printf("current state <%d%d%d%d> (depth %d)\n", a[0], a[1], a[2], a[3], depth);
		fprintf(stderr, "depth: %d init: %d goal: %d\n", depth, initial_state, goal_state);
		...
		if ( is_dead_end(initial_state) ){
		...
		else if ( is_visited(visited, depth, initial_state) ){
			...
			if (depth%2==0){
				...
				// --pw->
				if (a[1] != 1) dfs_main(initial_state+12, goal_state, depth+1, visited);
				// --pg->
				if (a[2] != 1) dfs_main(initial_state+10, goal_state, depth+1, visited);
				// --pc->
				// 0001 에서 pc는 못함. 
				if (a[3] != 1) dfs_main(initial_state+9, goal_state, depth+1, visited);
